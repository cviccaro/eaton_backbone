...
if (!class_exists('TCPDF', false)) {
	...
	*/
	class TCPDF {
		 ...
		 * @access public
		 */
		public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $margins = array(5,5,5,5)) {
			...
			//$margin = 28.35 / $this->k;
			$margin = $margins[0];
			//$this->SetMargins($margins[0], $margins[1]);
		...
		public function Header() {
			...
			$this->Cell(0, 0, '', 'T', 0, 'C');
			$this->Image('http://' . $_SERVER['HTTP_HOST'] . '/' . drupal_get_path('theme','eaton') . '/img/Eaton_Badge_BG.jpg', 0, 0, 1013, 638, '', '', false, 300, '', false, false, 0);
		}
		 ...
		 * @access public
		 * @since 1.0
		 * @see Cell(), Write(), MultiCell(), WriteHTML(), 
		 * Cell()
		 */
	...
	Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
		...
		public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
			watchdog(__FUNCTION__, 'Got to line ' . __LINE__ . ' of file ' . __FILE__ . ' in function ' . __FUNCTION__, NULL, WATCHDOG_INFO);
			$gvars = $this->getGraphicVars();
			// store current values
			$prevPage = $this->page;
